...
def add():
    ...
    if(num_where_add in range(1,len(space.room))):
        ...
        if (result == False):
            ...
            if(len(space.room[num_where_add]) == 1):
                return
            else:
                for z in range(0, len(space.room[num_where_add])*2):
                    for y in range(2, len(space.room[num_where_add])):
                        first_term = space.room[num_where_add][y].split(" |term: ")
                        second_term = space.room[num_where_add][y - 1].split(" |term: ")

                        s = first_term[1].split("-")
                        f = second_term[1].split("-")

                        if (s[0] < f[0]):
                            wait = space.room[num_where_add][y]
                            space.room[num_where_add][y] = space.room[num_where_add][y - 1]
                            space.room[num_where_add][y - 1] = wait
                            break
                        elif (s[0] == f[0]):
                            if (s[1] < f[1]):
                                wait = space.room[num_where_add][y]
        ...
        for x in range(1, len(space.room)):
            result = term()
            if(result == False):
                pass
            else:
                ...
                for y in range(1, len(space.room.x)):
                    pass                #continue !!!!!! This not all
